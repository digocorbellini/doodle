...
namespace Vec2Math
{
	...
	template<typename T>
	inline sf::Vector2<T> Scale( const sf::Vector2<T>& a, const T scalar )
	{
		return  sf::Vector2<T>( a.x * scalar, a.y * scalar );
	}

	template<typename T>
	inline T Magnitude( const sf::Vector2<T>& a )
	{
		return std::sqrt( ( a.x * a.x ) + ( a.y * a.y ) );
	}

	template<typename T>
	inline sf::Vector2<T> Normalize( const sf::Vector2<T>& a )
	{
	...
	template<typename T>
	inline bool Equals( const sf::Vector2<T>& a, const sf::Vector2<T>& b )
	{
